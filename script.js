...
function renderArticles() {
  ...
  // On first page, show trolley + quiz widgets in first row
  if (currentPage === 1 && articlesToShow.length > 0 && isHomePage) {
    // Create widget container row
  ...
  // Attach poll buttons only on homepage, after DOM elements exist
  if (isHomePage) {
...
function openModal(article) {
  ...
  // Split content into lines using newline character
  ...
  let html = '';
  lines.forEach(line => {
    const imgMatch = line.match(/^\[img:(.+?)\]$/i); // e.g. [img:https://example.com]
    if (imgMatch) {
      const imgUrl = imgMatch[1];
      html += `<img src="${imgUrl}" alt="Article Image" style="max-width: 100%; margin-bottom: 1rem;" />`;
    } else {
      html += `<p>${line}</p>`; // turn each text line into a <p>
    }
  });
...
function vote(option) {
  ...
  if(result) {
    result.textContent = `Pull the Lever: ${percentA}% | Do Nothing: ${percentB}%`;
  }
}
...
navLinks.forEach(link => {
  link.addEventListener('click', (e) => {
    ...
    if (genre === 'philosophy' || genre === 'ai and tech' || genre === 'tech for teens') {
      ...
      // Wait until articles are rendered
setTimeout(() => {
...
// Event listeners for search, sort, pagination, modal, dark mode toggle
